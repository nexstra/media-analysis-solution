...
class Result extends Component {
                    ...
                    if (result.hasOwnProperty('Next')) {
                        ...
                        if (self.state.file_type === 'mp4' || self.state.file_type === 'mov') {
                            for (var a in data.Attributes) {
                                att_video[data.Attributes[a].Name] = {
                                    "Name": data.Attributes[a].Name,
                                    "Impressions": {}
                                };
                                ...
                                for (var i in data.Attributes[a].Impressions) {
                                    ...
                                    if (att_video[data.Attributes[a].Name].Impressions.hasOwnProperty(Math.ceil((data.Attributes[a].Impressions[i].Timestamp) / 100) * 100)) {
                                        att_video[data.Attributes[a].Name].Impressions[Math.ceil((data.Attributes[a].Impressions[i].Timestamp) / 100) * 100][att_count] = {
                                            "BoundingBox": data.Attributes[a].Impressions[i].BoundingBox,
                                            "Confidence": data.Attributes[a].Impressions[i].Confidence
                                        };
                                    } else {
                            ...
                            if ('BoundingBox' in data.Persons.Persons[p].Person) {
                                if ((Math.ceil((data.Persons.Persons[p].Timestamp) / 100) * 100) in person_tracking) {
                                    if ((data.Persons.Persons[p].Person.Index in person_tracking[Math.ceil((data.Persons.Persons[p].Timestamp) / 100) * 100]) === false) {
                                        person_tracking[Math.ceil((data.Persons.Persons[p].Timestamp) / 100) * 100][data.Persons.Persons[p].Person.Index] = {
                                            "Index": data.Persons.Persons[p].Person.Index,
                                            "BoundingBox": data.Persons.Persons[p].Person.BoundingBox
                                        };
                                    }
                                } else {
                ...
                .then(function (result) {
                    for (var f in result.FaceMatches) {
                        if (face_matches.hasOwnProperty(result.FaceMatches[f].ExternalImageId) == false) {
                            face_matches[result.FaceMatches[f].ExternalImageId] = {
                                'ExternalImageId': result.FaceMatches[f].ExternalImageId,
                                'Impressions': []
                            };
                    ...
                    if (result.hasOwnProperty('Next')) {
                        ...
                        if (self.state.file_type === 'mp4' || self.state.file_type === 'mov') {
                            for (var m in data.FaceMatches) {
                                video_face_matches[data.FaceMatches[m].ExternalImageId] = {
                                    "Name": data.FaceMatches[m].ExternalImageId,
                                    "Impressions": {}
                                };
                                ...
                                for (var i in data.FaceMatches[m].Impressions) {
                                    if (data.FaceMatches[m].Impressions[i].Face.hasOwnProperty("BoundingBox")) {
                                        if (video_face_matches[data.FaceMatches[m].ExternalImageId].Impressions.hasOwnProperty(Math.ceil((data.FaceMatches[m].Impressions[i].Timestamp) / 100) * 100)) {
                                            video_face_matches[data.FaceMatches[m].ExternalImageId].Impressions[Math.ceil((data.FaceMatches[m].Impressions[i].Timestamp) / 100) * 100][data.FaceMatches[m].ExternalImageId] = {
                                                "Name": data.FaceMatches[m].ExternalImageId,
                                                "BoundingBox": data.FaceMatches[m].Impressions[i].Face.BoundingBox
                                            };
                                        } else {
                                        ...
                                        if ((Math.ceil((data.FaceMatches[m].Impressions[i].Timestamp) / 100) * 100) in video_known_faces) {
                                            if ((data.FaceMatches[m].ExternalImageId in video_known_faces[Math.ceil((data.FaceMatches[m].Impressions[i].Timestamp) / 100) * 100]) === false) {
                                                video_known_faces[Math.ceil((data.FaceMatches[m].Impressions[i].Timestamp) / 100) * 100][data.FaceMatches[m].ExternalImageId] = {
                                                    "Name": data.FaceMatches[m].ExternalImageId,
                                                    "BoundingBox": data.FaceMatches[m].Impressions[i].Face.BoundingBox
                                                };
                                            }
                            ...
                            for (var f in data.FaceMatches) {
                                ...
                                for (var i in data.FaceMatches[f].Impressions) {
                                    face_match_boxes.push({
                                        "ExternalImageId": data.FaceMatches[f].ExternalImageId,
                                        "FaceId": data.FaceMatches[f].Impressions[i].Face.FaceId,
                                        "BoundingBox": data.FaceMatches[f].Impressions[i].Face.BoundingBox
                                    })
                                }
                            }
                        }
                ...
                .then(function (result) {
                    ...
                    if (result.hasOwnProperty('Next')) {
                        ...
                        for (var l in data.Labels) {
                            ...
                            label_list.push({
                                "Name": data.Labels[l].Name,
                                "Confidence": confidence,
                                "Id": [data.Labels[l].Name.replace(/[^\w\s]|_/g, " ").replace(/\s+/g, " "), uuidv4()].join('-')
                            });
                        }
                        self.setState({
    ...
    getTextWords() {
        var self = this
        var texts_path = ['/lookup', this.props.match.params.objectid, 'texts'].join('/');
        var text_list = [];
        console.debug("TXT Lookups: getTextWords");
    ...
    getModeration() {
        var moderation_path = ['/lookup', this.props.match.params.objectid, 'moderations'].join('/');
        var moderation_list = [];
        console.debug("getModerationL:components/result.js")
        var self = this


        let _getModerations = function (path, labels) {
            console.debug("Sending path,labels to _getModerations at: " + path )
            API.get('MediaAnalysisApi', path, {})
                .then(function (result) {
                   console.debug("API Returend with moderations")
                    for (var l in result.ModerationLabels) {
                        console.debug( JSON.stringify(result.ModerationLabels[l]) )
                        var name = result.ModerationLabels[l].Name
                        if (labels.hasOwnProperty(name) == false) {
                            console.debug( name + " hasOwnProperty")

                            labels[name] = {'Name': name, 'Impressions': []};
                            labels[name].Impressions = labels[name].Impressions.concat(result.ModerationLabels[l].Impressions);
                        } else {
                            labels[name].Impressions = labels[name].Impressions.concat(result.ModerationLabels[l].Impressions);
                        }
                    }
                    if (result.hasOwnProperty('Next')) {
                        _getModerations([moderation_path, result.Next].join('?page='), labels);
                    } else {
                        console.debug( name + " !Next - collecting labels from:\n" + JSON.stringify(labels))

                        let data = {'ModerationLabels': []};
                        for (var i in labels) {
                            data.ModerationLabels.push(labels[i]);
                        }
                        for (var l in data.ModerationLabels) {
                            let confidence = 0;
                            for (var i in data.ModerationLabels[l].Impressions) {
                                if (data.ModerationLabels[l].Impressions[i].Confidence >= confidence) {
                                    confidence = data.ModerationLabels[l].Impressions[i].Confidence.toFixed(3);
                                }
                            }
                            moderation_list.push({
                                "Name": data.ModerationLabels[l].Name,
                                "Confidence": confidence,
                                "Id": [name.replace(/[^\w\s]|_/g, " ").replace(/\s+/g, " "), uuidv4()].join('-')
                            });
                        }
                        console.debug( name + " setting state to \n" + JSON.stringify(moderation_list))
        ...
        _getModerations(moderation_path
            , {});
    };
                ...
                .then(function (result) {
                    for (var c in result.Celebs) {
                        if (celebs.hasOwnProperty(result.Celebs[c].Id) == false) {
                            celebs[result.Celebs[c].Id] = {
                                'Id': result.Celebs[c].Id,
                                'Name': result.Celebs[c].Name,
                                'Impressions': []
                            };
                                ...
                                for (var i in data.Celebs[c].Impressions) {
                                    if (data.Celebs[c].Impressions[i].hasOwnProperty("BoundingBox")) {
                                        if (video_celeb_faces[data.Celebs[c].Id].Impressions.hasOwnProperty(Math.ceil((data.Celebs[c].Impressions[i].Timestamp) / 100) * 100)) {
                                            video_celeb_faces[data.Celebs[c].Id].Impressions[Math.ceil((data.Celebs[c].Impressions[i].Timestamp) / 100) * 100][data.Celebs[c].Id] = {
                                                "Name": data.Celebs[c].Name,
                                                "BoundingBox": data.Celebs[c].Impressions[i].BoundingBox
                                            };
                                        } else {
                                        ...
                                        if ((Math.ceil((data.Celebs[c].Impressions[i].Timestamp) / 100) * 100) in celeb_video) {
                                            if ((data.Celebs[c].Name in celeb_video[Math.ceil((data.Celebs[c].Impressions[i].Timestamp) / 100) * 100]) === false) {
                                                celeb_video[Math.ceil((data.Celebs[c].Impressions[i].Timestamp) / 100) * 100][data.Celebs[c].Name] = {
                                                    "Name": data.Celebs[c].Name,
                                                    "BoundingBox": data.Celebs[c].Impressions[i].BoundingBox
                                                };
                                            }
        ...
        API.get('MediaAnalysisApi', entities_path, {})
            .then(function (data) {
                for (var e in data.Entities) {
                    entity_list.push({
                        "Name": data.Entities[e].Name,
                        "Confidence": data.Entities[e].Impressions[0].Score * 100,
                        "Id": [data.Entities[e].Name.replace(/[^\w\s]|_/g, " ").replace(/\s+/g, " "), uuidv4()].join('-')
                    });
                }
                self.setState({
        ...
        API.get('MediaAnalysisApi', phrases_path, {})
            .then(function (data) {
                for (var p in data.Phrases) {
                    phrase_list.push({
                        "Name": data.Phrases[p].Name,
                        "Confidence": data.Phrases[p].Impressions[0].Score * 100,
                        "Id": [data.Phrases[p].Name.replace(/[^\w\s]|_/g, " ").replace(/\s+/g, " "), uuidv4()].join('-')
                    });
                }
                self.setState({
    ...
    render() {
        ...
        if (this.state) {
            ...
            <
                Button
                id = {label.Id.replace(/\s+/g, '-').toLowerCase()
            }
                color = "secondary"
                className = "ml-1 mr-1 mb-1 mt-1" > {label.Name
            }<
                /Button>
                < UncontrolledTooltip
                placement = "top"
                target = {label.Id.replace(/\s+/g, '-').toLowerCase()
            }>
                {
                    label.Confidence
                }
            ...
            <
                Button
                id = {phrase.Id.replace(/\s+/g, '-').toLowerCase()
            }
                color = "secondary"
                className = "ml-1 mr-1 mb-1 mt-1" > {phrase.Name
            }<
                /Button>
                < UncontrolledTooltip
                placement = "top"
                target = {phrase.Id.replace(/\s+/g, '-').toLowerCase()
            }>
                {
            ...
            <
                Button
                id = {entity.Id.replace(/\s+/g, '-').toLowerCase()
            }
                color = "secondary"
                className = "ml-1 mr-1 mb-1 mt-1" > {entity.Name
            }<
                /Button>
                < UncontrolledTooltip
                placement = "top"
                target = {entity.Id.replace(/\s+/g, '-').toLowerCase()
            }>
                {
            ...
            <
                Button
                id = {text.Id.replace(/\s+/g, '-').toLowerCase()
            }
                color = "secondary"
                className = "ml-1 mr-1 mb-1 mt-1" > {text.Name
            }<
                /Button>
                < UncontrolledTooltip
                placement = "top"
                target = {text.Id.replace(/\s+/g, '-').toLowerCase()
            }>
                {
            ...
            if (this.state.file_type === 'png' || this.state.file_type === 'jpg' || this.state.file_type === 'jpeg') {
                ...
                name = "error"
                color = "danger"
                isOpen = {this.state.error_status
            }
                toggle = {this.Dismiss
            }>
                Error
            ...
            <
                ...
                value = "100" / >
                    ...
                    < /Modal>
                    < ImageResults
                allfaces = {this.state.face_list
            }
                knownfaces = {this.state.known_faces
            }
                mediafile = {this.state.media_file
            }
                ...
                labels = {labels}
                celebfaces = {this.state.celeb_faces
            }
                attlist = {this.state.att_list
            }
                />
                < /div>
            ...
            <
                ...
                individualcelebs = {this.state.video_indv_celebs
            }
                allfaces = {this.state.face_video
            }
                attributes = {this.state.att_list
            }
                celebvideo = {this.state.celeb_video
            }
                mediafile = {this.state.media_file
            }
                filename = {this.state.filename
                ...
                labels = {labels}
                individualknownfaces = {this.state.video_indv_known_faces
            }
                allknownfaces = {this.state.known_face_video
            }
                />
                < /div>
            )
